...
for i=1:size(data_per_3,1)
   ...
   baseline__matched = baseline_per_3(randperm(size(baseline_per_3, 1), size(baseline_aper_3, 1)), :);
    ...
    mean_diff(1,i)=mean(per_3_matched_minus_aper_3(:,9:33),2);
    ...
    %ttest
    norm_mean_diff(1,i)=mean(norm_selectivity(:,9:33),2);
    [h,p,ci,stats] = ttest2(baseline__matched,per_3_matched_minus_aper_3(:,9:33));
    %save the results
    ttest_results_per_3(i,1)=h; %h=1 means significant  
    ttest_results_per_3(i,2)=p;
% ttest for normmalised 
    [h,p,ci,stats] = ttest(norm_selectivity(:,9:33),norm_baseline__matched);
...
for i=1:size(data_per_4,1)
   ...
   baseline__matched = baseline_per_4(randperm(size(baseline_per_4, 1), size(baseline_aper_4, 1)), :);
    ...
    mean_diff(1,i)=mean(per_4_matched_minus_aper_4(:,9:33),2);
    ...
    norm_mean_diff(1,i)=mean(norm_selectivity(:,9:33),2);
    %ttest
    [h,p,ci,stats] = ttest2(baseline__matched,per_4_matched_minus_aper_4(:,9:33));
    %save the results
...
figure;
 ...
 hold on;
 plot(norm_mean_diff);
 legend('mean_diff','norm_mean_diff');
 xlabel('cell no');
 ylabel('Periodic-Aperiodic response difference');
title('Periodic-Aperiodic response difference for periodicity 4 ');